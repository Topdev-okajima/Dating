	...
	function initDataTableHelper () {
		if ($.fn.dataTable) {
			$('[data-provide="datatable"]').each (function () {	
				...
				if (helperOptions.sAjaxSource) {tableConfig.sAjaxSource = helperOptions.sAjaxSource;}
				
				if (helperOptions.fnRowCallback) 
				{
					eval('tableConfig.fnRowCallback = function( nRow, aData, iDisplayIndex ){ return ' + helperOptions.fnRowCallback + '(nRow, aData, iDisplayIndex);};');
				}
				
				if (helperOptions.fnCreatedRow) 
				{
					eval('tableConfig.fnCreatedRow = function( nRow, aData, iDisplayIndex ){ return ' + helperOptions.fnCreatedRow + '(nRow, aData, iDisplayIndex);};');
				}
				
				if (helperOptions.fnDrawCallback) 
				{
					eval('tableConfig.fnDrawCallback = function(oSettings){ return ' + helperOptions.fnDrawCallback + '(oSettings);};');
				}
				...
				if (helperOptions.filterValue && helperOptions.filterValue != ''){
					$thisTable.fnFilter(helperOptions.filterValue);
				}
...
function getCorrespondingId(srcID, replacePart, replaceWith){
	return srcID.replace(replacePart, replaceWith);
}
